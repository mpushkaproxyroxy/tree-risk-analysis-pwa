...
function App() {
  ...
  const liveResult = useMemo(
    () => buildWhatIfScenario(result, liveWindSpeed),
    [result, liveWindSpeed]
  );

  function handleScenarioChange(next: ScenarioInput) {
    const withSpeciesDefaults =
      next.species !== draft.species
        ? hydrateSpeciesDefaults({
            ...next,
            windSpeedMs: next.windSpeedMs,
          })
        : next;
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <div className="mb-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600 sm:text-base">
            Set up a tree scenario, analyze it instantly in the browser, and
            review the result without leaving this page.
          </p>
        </div>
